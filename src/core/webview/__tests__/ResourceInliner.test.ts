	...
	describe("getInlinedResourcesData", () => {
		it("should return inlined resources data", async () => {
			...
			mockedFs.readFile
				...
				.mockResolvedValueOnce(Buffer.from("font-data")) // codicon.ttf
				.mockResolvedValueOnce(Buffer.from("notification-audio")) // notification.wav
				.mockResolvedValueOnce(Buffer.from("celebration-audio")) // celebration.wav
				.mockResolvedValueOnce(Buffer.from("progress-audio")) // progress_loop.wav
			...
			expect(result.codiconTtf).toContain("data:font/ttf;base64,")
			expect(result.notificationWav).toContain("data:audio/wav;base64,")
			expect(result.celebrationWav).toContain("data:audio/wav;base64,")
			expect(result.progressLoopWav).toContain("data:audio/wav;base64,")
		})
		...
		it("should handle missing files gracefully", async () => {
			...
			expect(result.codiconTtf).toBe("")
			expect(result.notificationWav).toBe("")
			expect(result.celebrationWav).toBe("")
			expect(result.progressLoopWav).toBe("")
		})
	})
})
