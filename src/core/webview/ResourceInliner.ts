...
export class ResourceInliner {
	...
	async getInlinedResourcesData(): Promise<{
		...
		requestyPng: string
		codiconTtf: string
		notificationWav: string
		celebrationWav: string
		progressLoopWav: string
	}> {
		const resources = {
			rooLogoSvg: "",
			openrouterPng: "",
		...
		// Inline font file
		if (await this.fileExists(["assets", "codicons", "codicon.ttf"])) {
			resources.codiconTtf = await this.imageToDataUri(["assets", "codicons", "codicon.ttf"], "font/ttf")
		}

		// Inline audio files
		if (await this.fileExists(["webview-ui", "audio", "notification.wav"])) {
			resources.notificationWav = await this.imageToDataUri(["webview-ui", "audio", "notification.wav"], "audio/wav")
		}

		if (await this.fileExists(["webview-ui", "audio", "celebration.wav"])) {
			resources.celebrationWav = await this.imageToDataUri(["webview-ui", "audio", "celebration.wav"], "audio/wav")
		}

		if (await this.fileExists(["webview-ui", "audio", "progress_loop.wav"])) {
			resources.progressLoopWav = await this.imageToDataUri(["webview-ui", "audio", "progress_loop.wav"], "audio/wav")
		}
